...
export function rankToday(todos: Todo[], events: CalEvent[], today: string) {
  const projectsWithEventToday = new Set<string>(); // reserved: needs event<->project link, unused for now
  void events;
  void projectsWithEventToday;
...
export function openingMessage(ctx: PlanContext): string {
  ...
  const ranked = rankToday(ctx.todos, ctx.events, today);
  const pname = (id: string | null) =>
    id ? (ctx.projects.find((p) => p.id === id)?.name ?? "—") : "no project";

  if (ranked.length === 0 && ctx.events.length === 0) {
    return "Good " + dayPart() + ". Nothing's pressing today — clear list, no events. Anything you want to line up for later this week?";
  }

  const lines: string[] = [];
  ranked.slice(0, 6).forEach((t, i) => {
    const tag = t.due_date && t.due_date < today ? "overdue" : t.due_date === today ? "due today" : t.flagged ? "flagged" : "";
    lines.push(`${i + 1}. ${t.title} — ${pname(t.project_id)}${tag ? ` (${tag})` : ""}`);
  });

  const evLine = ctx.events.length
    ? `\n\nOn the calendar: ${ctx.events
        .map((e) => `${e.allDay ? "all day" : new Date(e.start).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })} ${e.title}`)
        .join(", ")}.`
    : "";

  return `Good ${dayPart()}. Here's the order I'd go in:\n\n${lines.join("\n")}${evLine}\n\nWhat do you want to start with?`;
}
...
export function buildSystemPrompt(ctx: PlanContext): string {
  ...
  return `You are Haru, a calm daily planning assistant for one person. Today is ${today}.
...
- Dates are YYYY-MM-DD. Event times are full ISO 8601 with timezone offset.
- If the user asks for something you have no tool for (e.g. deleting an event), tell them to do it themselves in Google Calendar.
...
${eventLines}
${ctx.googleConnected ? "" : "\n(Calendar is not connected — the create_event / move_event tools are unavailable.)"}`;
}
